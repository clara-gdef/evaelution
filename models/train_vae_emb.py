...
def get_collate_fn_and_class_nums(hparams):
    if hparams.att_type == "both":
        return collate_for_VAE_emb, 20, 3
    elif hparams.att_type == "exp":
        return collate_for_VAE_emb_exp, 0, 3
    elif hparams.att_type == "ind":
        return collate_for_VAE_emb_ind, 20, 0
    else:
        raise Exception(f"Wrong att_type specified. Can be exp, ind, or both. Got: {hparams.att_type}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--att_type", type=str, default="both") # can be both, exp, ind or "none"
    # model attributes
    ...
    parser.add_argument("--model_type", type=str, default="VAEemb")
    # global hyper params
